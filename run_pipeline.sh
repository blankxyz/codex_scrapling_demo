...
GEN_PROMPT="使用当前目录的 \$scrapling-spider-generator，根据 ${ANALYSIS_JSON} 里的分析结果生成 Scrapling 爬虫，输出到 ${NEW_SPIDER}。只抓第一页。必须直接把代码写入这个文件路径；如果没有落盘到该文件就算失败。"
...
if [[ -z "$FLOW_FUNC" ]]; then
  ...
  if grep -qF "$ENTRY" "$DEPLOY_REGISTRY"; then
    ...
    ANCHOR_TIME=$(next_anchor_time "$DEPLOY_REGISTRY")
