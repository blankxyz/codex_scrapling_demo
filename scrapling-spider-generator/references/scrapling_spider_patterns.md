...
Preserve the user's schema exactly. The key order is part of the schema. Do not add, remove, rename, or reorder fields. When a site has mixed text and video details, generate separate builders and branch in `parse_detail` by `response.meta["content_type"]` or the detail URL path.
