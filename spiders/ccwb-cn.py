...
class CcwbCnSpider:
    SITE_SLUG = "ccwb-cn"
    SITE_NAME = "长城网"
    LIST_URL = "https://www.ccwb.cn/col459.html"
    TARGET_HOST = "www.ccwb.cn"

    # Marker primary selector first, then robust fallbacks.
    LIST_LINK_SELECTORS = [
        "html > body > div.kpContentBox > div.wrap > div.kpConBoxC a",
        "div.kpConBoxC a",
        "a[href*='/content/']",
    ]
    ...
    ARTICLE_URL_PATTERN = re.compile(r"/content/\d{6}/\d{2}/c\d+\.html(?:$|[?#])", re.I)
    LIST_URL_PATTERN = re.compile(r"/col459(?:_\d+)?\.html(?:$|[?#])", re.I)
    ...
    def _normalize_url(cls, href: str, base_url: str) -> str:
        ...
        if not parsed.scheme or not parsed.netloc:
            return ""
        # For same-host detail/list links, force https to avoid redirect edge cases.
        if parsed.netloc.endswith("ccwb.cn") and parsed.scheme == "http":
        ...
        for selector in cls.LIST_LINK_SELECTORS:
            ...
            for node in nodes:
                ...
                for anchor in anchors:
                    ...
                    if not parsed.netloc.endswith("ccwb.cn"):
                        continue
    ...
    def _extract_list_page_urls(cls, html: str, base_url: str) -> list[str]:
        ...
        for a in doc.css("a[href]"):
            ...
            if not parsed.netloc.endswith("ccwb.cn"):
                continue
            if not cls.LIST_URL_PATTERN.search(parsed.path):
                continue
            if url in seen:
                continue
            seen.add(url)
    ...
    def parse_list_page(cls, url: str | None = None, dynamic: bool = False) -> list[str]:
        ...
        # Retry with JS rendering only if static fetch found no links.
    ...
    def parse_detail_html(cls, html: str, url: str) -> dict[str, Any]:
        ...
        if not content_html and content_text:
            # Keep required schema stable when page only exposes plain text.
    ...
    def parse_detail_page(cls, url: str, dynamic: bool = False) -> dict[str, Any]:
        ...
        if (item.get("title") and item.get("content_text")) or dynamic:
            return item
...
if __name__ == "__main__":
    ...
    result = CcwbCnSpider.crawl_all(dynamic=False)
    output_path = "out/ccwb-cn/crawl_results.json"
